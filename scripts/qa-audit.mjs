...
const COLLECTIONS = ['guides', 'compare', 'areas', 'projects', 'developers', 'news', 'segments', 'move-to-portugal'];
...
function auditFile(c, slug) {
  ...
  const internal = links.filter((l) =>
    /\]\(\/(guides|compare|areas|projects|developers|news|segments|move-to-portugal)\//i.test(l),
  );
